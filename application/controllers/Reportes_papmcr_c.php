...
class Reportes_papmcr_c extends CI_Controller {

	public function __construct()
	{
		parent::__construct();
		$this->load->helper("url","form");
		$this->load->model('Reportes_papmcr_m');
	...
	public function index()
	{
		...
		$data['mes']= $this->Reportes_papmcr_m->mostrar();
		$data['red']= $this->Reportes_papmcr_m->red();
		$data['annio']= $this->Reportes_papmcr_m->mostrar_annio();
		...
		if ($entra=='1') {
			$this->load->view('exportar/Reportes_papmcr_v',$data);
		}else{
	...
	function microred(){
		$microred = $this->Reportes_papmcr_m->microred($this->input->post("id"));
	...
	public function exportar_mes()
	{
		...
		$sitio=7;
		$esta = $this->db->query("select ipress.codigo,microred.id_microred,microred.microred,red_salud.red_salud FROM  ipress inner join microred on ipress.microred=microred.id_microred INNER JOIN red_salud on microred.red =red_salud.id_red where red_salud.id_red=".$this->input->post("red")." group by microred.microred ORDER BY microred.id_microred")->result();

		foreach ($esta as  $val) {
			$nombre_microred=$val->red_salud;
			$ipress= ($val->id_microred);
			$objPHPExcel->getActiveSheet()->setCellValue('A'.$sitio, $val->microred);
			...
			$nega1 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad , fecha_muestra FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where  clasificacion_general like 'Negativo para lesiones epiteliales o malignidad' and  TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())<=29 and TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=15  and id_mesr=".$this->input->post("mes_seleccion")." and  microred.id_microred=".$ipress)->result();
			foreach ($nega1 as  $values) {	
				$i=	$values->cantidad +$i;

			}
			$nega2 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos  INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where  clasificacion_general='Negativo para lesiones epiteliales o malignidad' and  TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())<=49 and TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=30  and id_mesr=".$this->input->post("mes_seleccion")." and microred.id_microred=".$ipress )->result();
			foreach ($nega2 as  $values) {	
				$i1=	$values->cantidad +$i1;
			}
			$nega3 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred  where  clasificacion_general='Negativo para lesiones epiteliales o malignidad' and TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=50 and id_mesr=".$this->input->post("mes_seleccion")." and microred.id_microred=".$ipress )->result();
			foreach ($nega3 as  $values) {	
				$i2=	$values->cantidad +$i2;
			}
			$agus1 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where   TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())<=29 and TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=15  and id_mesr=".$this->input->post("mes_seleccion")." and  microred.id_microred=".$ipress." and celulas_glandulares_atipicas!='_' or celulas_glandulares_atipicas!=  NULL " )->result();
			foreach ($agus1 as  $values) {	
				$agus_c1=	$values->cantidad +$agus_c1;
			}
			$agus2 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where  TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())<=49 and TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=30 and id_mesr=".$this->input->post("mes_seleccion")." and microred.id_microred=".$ipress." and celulas_glandulares_atipicas!='_' or celulas_glandulares_atipicas!=  NULL " )->result();
			foreach ($agus2 as  $values) {	
				$agus_c2=	$values->cantidad +$agus_c2;
			}
			$agus3 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where  TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=50 and id_mesr=".$this->input->post("mes_seleccion")." and microred.id_microred=".$ipress." and celulas_glandulares_atipicas!='_' or celulas_glandulares_atipicas!=  NULL " )->result();
			foreach ($agus3 as  $values) {	
				$agus_c3=	$values->cantidad +$agus_c3;
			}
			$ascus1 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where   TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())<=29 and TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=15 and id_mesr=".$this->input->post("mes_seleccion")." and microred.id_microred=".$ipress." and celulas_escamosas_atipicas!='_' or celulas_escamosas_atipicas!=  NULL " )->result();
			foreach ($ascus1 as  $values) {	
				$ascus_c1=	$values->cantidad +$ascus_c1;
			}
			$ascus2 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where  TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())<=49 and TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=30 and id_mesr=".$this->input->post("mes_seleccion")." and microred.id_microred=".$ipress." and celulas_escamosas_atipicas!='_' or celulas_escamosas_atipicas!=  NULL " )->result();
			foreach ($ascus2 as  $values) {	
				$ascus_c2=	$values->cantidad +$ascus_c2;
			}
			$ascus3 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where  TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=50 and id_mesr=".$this->input->post("mes_seleccion")."  and microred.id_microred=".$ipress." and celulas_escamosas_atipicas!='_' or celulas_escamosas_atipicas!=  NULL " )->result();
			foreach ($ascus3 as  $values) {	
				$ascus_c3=	$values->cantidad +$ascus_c3;
			}
			$leigb1 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where   TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())<=29 and TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=15 and id_mesr=".$this->input->post("mes_seleccion")."  and microred.id_microred=".$ipress." and leibg!='_' or leibg!=  NULL " )->result();
			foreach ($leigb1 as  $values) {	
				$leibg_c1=	$values->cantidad +$leibg_c1;
			}
			$leigb2 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where  TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())<=49 and TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=30 and id_mesr=".$this->input->post("mes_seleccion")." and microred.id_microred=".$ipress." and leibg!='_' or leibg!=  NULL " )->result();
			foreach ($leigb2 as  $values) {	
				$leibg_c2=	$values->cantidad +$leibg_c2;
			}
			$leigb3 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where  TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=50 and id_mesr=".$this->input->post("mes_seleccion")." and microred.id_microred=".$ipress." and leibg!='_' or leibg!=  NULL " )->result();
			foreach ($leigb3 as  $values) {	
				$leibg_c3=	$values->cantidad +$leibg_c3;
			}
			$leiga1 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where   TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())<=29 and TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=15 and id_mesr=".$this->input->post("mes_seleccion")." and microred.id_microred=".$ipress." and leiag!='_' or leiag!=  NULL " )->result();
			foreach ($leiga1 as  $values) {	
				$leiag_c1=	$values->cantidad +$leiag_c1;
			}
			$leiga2 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where  TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())<=49 and TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=30  and id_mesr=".$this->input->post("mes_seleccion")." and microred.id_microred=".$ipress." and leiag!='_' or leiag!= NULL " )->result();
			foreach ($leiga2 as  $values) {	
				$leiag_c2=	$values->cantidad +$leiag_c2;
			}
			$leiga3 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where  TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=50 and id_mesr=".$this->input->post("mes_seleccion")." and microred.id_microred=".$ipress." and leiag!='_' or leiag!= NULL " )->result();
			foreach ($leiga3 as  $values) {	
				$leiag_c3=	$values->cantidad +$leiag_c3;
			}
			$rechazo1 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where   TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())<=29 and TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=15 and id_mesr=".$this->input->post("mes_seleccion")." and microred.id_microred=".$ipress." and fecha_rechazo!=  '' " )->result();
			foreach ($rechazo1 as  $values) {	
				$rechazo_c1=	$values->cantidad +$rechazo_c1;
			}
			$rechazo2 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where  TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())<=49 and TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=30 and id_mesr=".$this->input->post("mes_seleccion")." and microred.id_microred=".$ipress." and fecha_rechazo!= '' " )->result();
			foreach ($rechazo2 as  $values) {	
				$rechazo_c2=	$values->cantidad +$rechazo_c2;
			}
			$rechazo3 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where  TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=50 and id_mesr=".$this->input->post("mes_seleccion")."  and microred.id_microred=".$ipress." and fecha_rechazo!='' " )->result();
			foreach ($rechazo3 as  $values) {	
				$rechazo_c3=	$values->cantidad +$rechazo_c3;
			}
			$esta_total = $this->db->query("select count(datos.codigo_renipres) as cantidad FROM  datos  INNER JOIN ipress ON datos.codigo_renipres = ipress.codigo INNER JOIN microred on microred.id_microred=ipress.microred where id_mesr=".$this->input->post("mes_seleccion")."  and microred.id_microred=".$ipress)->result();	
		...
		$objPHPExcel->getActiveSheet()->setCellValue('A2', 'MICROREDS DE LA RED '.$nombre_microred.' MES DE '.$nombre_mes.' '.$annio);
	...
	public function exportar_annio()
	{
		...
		$pos_mes = array();
		$cant=$this->db->query("select count(ipress.ipress) as cantidad FROM ipress  inner join microred on ipress.microred=microred.id_microred inner join red_salud on microred.id_microred=red_salud.id_red where red_salud.id_red=".$this->input->post('red_a') )->row();
		$pos_cant=1;
		for ($i=0; $i <=12 ; $i++) { 
		...
		for ($mes=1; $mes <=13 ; $mes++) { 
			...
			$esta = $this->db->query("select ipress.codigo,microred.id_microred,microred.microred,red_salud.red_salud FROM  ipress inner join microred on ipress.microred=microred.id_microred INNER JOIN red_salud on microred.red =red_salud.id_red where red_salud.id_red=".$this->input->post("red_a")." group by microred.microred ORDER BY microred.id_microred")->result();
			$pos=6;
			foreach ($esta as  $val) {
				$i=0;$i1=0;$i2=0;$agus_c1=0;$agus_c2=0;$agus_c3=0;$ascus_c1=0;$ascus_c2=0;$ascus_c3=0;$leibg_c1=0;$leibg_c2=0;$leibg_c3=0;$leiag_c1=0;$leiag_c2=0;$leiag_c3=0;$rechazo_c1=0;$rechazo_c2=0;$rechazo_c3=0;
				$nombre_microred=$val->red_salud;
				$ipress= ($val->id_microred);
				$objPHPExcel->getActiveSheet()->setCellValue('A'.($sitio+$pos), $val->microred);
				$nega1 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad , fecha_muestra FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where  clasificacion_general like 'Negativo para lesiones epiteliales o malignidad' and  TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())<=29 and TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=15  and id_mesr=".$this->input->post("annio_seleccion").$mes." and  microred.id_microred=".$ipress )->result();
				foreach ($nega1 as  $values) {	
					$i=	$values->cantidad +$i;
				}
				$nega2 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos  INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where  clasificacion_general='Negativo para lesiones epiteliales o malignidad' and  TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())<=49 and TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=30  and id_mesr=".$this->input->post("annio_seleccion").$mes." and microred.id_microred=".$ipress )->result();
				foreach ($nega2 as  $values) {	
					$i1=	$values->cantidad +$i1;
				}
				$nega3 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where  clasificacion_general='Negativo para lesiones epiteliales o malignidad' and TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=50 and id_mesr=".$this->input->post("annio_seleccion").$mes." and microred.id_microred=".$ipress )->result();
				foreach ($nega3 as  $values) {	
					$i2=	$values->cantidad +$i2;
				}
				$agus1 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where   TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())<=29 and TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=15  and id_mesr=".$this->input->post("annio_seleccion").$mes." and  microred.id_microred=".$ipress." and celulas_glandulares_atipicas!='_' or celulas_glandulares_atipicas!=  NULL " )->result();
				foreach ($agus1 as  $values) {	
					$agus_c1=	$values->cantidad +$agus_c1;
				}
				$agus2 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where  TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())<=49 and TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=30 and id_mesr=".$this->input->post("annio_seleccion").$mes." and microred.id_microred=".$ipress." and celulas_glandulares_atipicas!='_' or celulas_glandulares_atipicas!=  NULL " )->result();
				foreach ($agus2 as  $values) {	
					$agus_c2=	$values->cantidad +$agus_c2;
				}
				$agus3 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where  TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=50 and id_mesr=".$this->input->post("annio_seleccion").$mes." and microred.id_microred=".$ipress." and celulas_glandulares_atipicas!='_' or celulas_glandulares_atipicas!=  NULL " )->result();
				foreach ($agus3 as  $values) {	
					$agus_c3=	$values->cantidad +$agus_c3;
				}
				$ascus1 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where   TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())<=29 and TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=15 and id_mesr=".$this->input->post("annio_seleccion").$mes." and microred.id_microred=".$ipress." and celulas_escamosas_atipicas!='_' or celulas_escamosas_atipicas!=  NULL " )->result();
				foreach ($ascus1 as  $values) {	
					$ascus_c1=	$values->cantidad +$ascus_c1;
				}
				$ascus2 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where  TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())<=49 and TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=30 and id_mesr=".$this->input->post("annio_seleccion").$mes." and microred.id_microred=".$ipress." and celulas_escamosas_atipicas!='_' or celulas_escamosas_atipicas!=  NULL " )->result();
				foreach ($ascus2 as  $values) {	
					$ascus_c2=	$values->cantidad +$ascus_c2;
				}
				$ascus3 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where  TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=50 and id_mesr=".$this->input->post("annio_seleccion").$mes."  and microred.id_microred=".$ipress." and celulas_escamosas_atipicas!='_' or celulas_escamosas_atipicas!=  NULL " )->result();
				foreach ($ascus3 as  $values) {	
					$ascus_c3=	$values->cantidad +$ascus_c3;
				}
				$leigb1 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where   TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())<=29 and TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=15 and id_mesr=".$this->input->post("annio_seleccion").$mes."  and microred.id_microred=".$ipress." and leibg!='_' or leibg!=  NULL " )->result();
				foreach ($leigb1 as  $values) {	
					$leibg_c1=	$values->cantidad +$leibg_c1;
				}
				$leigb2 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where  TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())<=49 and TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=30 and id_mesr=".$this->input->post("annio_seleccion").$mes." and microred.id_microred=".$ipress." and leibg!='_' or leibg!=  NULL " )->result();
				foreach ($leigb2 as  $values) {	
					$leibg_c2=	$values->cantidad +$leibg_c2;
				}
				$leigb3 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where  TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=50 and id_mesr=".$this->input->post("annio_seleccion").$mes." and microred.id_microred=".$ipress." and leibg!='_' or leibg!=  NULL " )->result();
				foreach ($leigb3 as  $values) {	
					$leibg_c3=	$values->cantidad +$leibg_c3;
				}
				$leiga1 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where   TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())<=29 and TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=15 and id_mesr=".$this->input->post("annio_seleccion").$mes." and microred.id_microred=".$ipress." and leiag!='_' or leiag!=  NULL " )->result();
				foreach ($leiga1 as  $values) {	
					$leiag_c1=	$values->cantidad +$leiag_c1;
				}
				$leiga2 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where  TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())<=49 and TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=30  and id_mesr=".$this->input->post("annio_seleccion").$mes." and microred.id_microred=".$ipress." and leiag!='_' or leiag!= NULL " )->result();
				foreach ($leiga2 as  $values) {	
					$leiag_c2=	$values->cantidad +$leiag_c2;
				}
				$leiga3 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where  TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=50 and id_mesr=".$this->input->post("annio_seleccion").$mes." and microred.id_microred=".$ipress." and leiag!='_' or leiag!= NULL " )->result();
				foreach ($leiga3 as  $values) {	
					$leiag_c3=	$values->cantidad +$leiag_c3;
				}
				$rechazo1 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where   TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())<=29 and TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=15 and id_mesr=".$this->input->post("annio_seleccion").$mes." and microred.id_microred=".$ipress." and fecha_rechazo!=  '' " )->result();
				foreach ($rechazo1 as  $values) {	
					$rechazo_c1=	$values->cantidad +$rechazo_c1;
				}
				$rechazo2 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where  TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())<=49 and TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=30 and id_mesr=".$this->input->post("annio_seleccion").$mes." and microred.id_microred=".$ipress." and fecha_rechazo!= '' " )->result();
				foreach ($rechazo2 as  $values) {	
					$rechazo_c2=	$values->cantidad +$rechazo_c2;
				}
				$rechazo3 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where  TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=50 and id_mesr=".$this->input->post("annio_seleccion").$mes."  and microred.id_microred=".$ipress." and fecha_rechazo!='' " )->result();
				foreach ($rechazo3 as  $values) {	
					$rechazo_c3=	$values->cantidad +$rechazo_c3;
				}
				$esta_total = $this->db->query("select count(datos.codigo_renipres) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where id_mesr=".$this->input->post("annio_seleccion").$mes."  and microred.id_microred=".$ipress)->result();	
				foreach ($esta_total as  $value_total) {
	...
	function validar_mes(){
		$mes = $this->Reportes_papmcr_m->validar_mes($this->input->post("id"),$this->input->post("id_mes"));
		echo json_encode($mes);
	}

	function validar_annio(){
		$annio = $this->Reportes_papmcr_m->validar_annio($this->input->post("id"));
	...
	function rango_mes(){
		...
		$pos_mes = array();
		$cant_r=$this->db->query("select count(ipress.ipress) as cantidad FROM ipress  inner join microred on ipress.microred=microred.id_microred inner join red_salud on microred.id_microred=red_salud.id_red where red_salud.id_red=".$this->input->post('red_r') )->row();
		$pos_cant=1;
		for ($i=0; $i <=12 ; $i++) { 
			array_push($pos_mes,$pos_cant );
		...
		for ($mes=1; $mes <=$cant+1 ; $mes++) { 
			...
			$objPHPExcel->getActiveSheet()->setCellValue('AB'.($sitio+4), '30-49');
			$objPHPExcel->getActiveSheet()->setCellValue('AC'.($sitio+4), '+ 50');
			$esta = $this->db->query("select ipress.codigo,microred.id_microred,microred.microred,red_salud.red_salud FROM  ipress inner join microred on ipress.microred=microred.id_microred INNER JOIN red_salud on microred.red =red_salud.id_red where red_salud.id_red=".$this->input->post("red_r")." group by microred.microred ORDER BY microred.id_microred")->result();
			$pos=6;
			foreach ($esta as  $val) {
				...
				$objPHPExcel->getActiveSheet()->setCellValue('A'.($sitio+$pos), $val->microred);
				if ($mes<=$cant) {
				$nega1 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad , fecha_muestra FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where  clasificacion_general like 'Negativo para lesiones epiteliales o malignidad' and  TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())<=29 and TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=15  and id_mesr=".$this->input->post("annio_r").$mes_ini." and  microred.id_microred=".$ipress )->result();
				foreach ($nega1 as  $values) {	
					$i=	$values->cantidad +$i;
				}
				$nega2 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos  INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where  clasificacion_general='Negativo para lesiones epiteliales o malignidad' and  TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())<=49 and TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=30  and id_mesr=".$this->input->post("annio_r").$mes_ini." and microred.id_microred=".$ipress )->result();
				foreach ($nega2 as  $values) {	
					$i1=	$values->cantidad +$i1;
				}
				$nega3 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where  clasificacion_general='Negativo para lesiones epiteliales o malignidad' and TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=50 and id_mesr=".$this->input->post("annio_r").$mes_ini." and microred.id_microred=".$ipress )->result();
				foreach ($nega3 as  $values) {	
					$i2=	$values->cantidad +$i2;
				}
				$agus1 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where   TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())<=29 and TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=15  and id_mesr=".$this->input->post("annio_r").$mes_ini." and  microred.id_microred=".$ipress." and celulas_glandulares_atipicas!='_' or celulas_glandulares_atipicas!=  NULL " )->result();
				foreach ($agus1 as  $values) {	
					$agus_c1=	$values->cantidad +$agus_c1;
				}
				$agus2 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where  TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())<=49 and TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=30 and id_mesr=".$this->input->post("annio_r").$mes_ini." and microred.id_microred=".$ipress." and celulas_glandulares_atipicas!='_' or celulas_glandulares_atipicas!=  NULL " )->result();
				foreach ($agus2 as  $values) {	
					$agus_c2=	$values->cantidad +$agus_c2;
				}
				$agus3 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where  TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=50 and id_mesr=".$this->input->post("annio_r").$mes_ini." and microred.id_microred=".$ipress." and celulas_glandulares_atipicas!='_' or celulas_glandulares_atipicas!=  NULL " )->result();
				foreach ($agus3 as  $values) {	
					$agus_c3=	$values->cantidad +$agus_c3;
				}
				$ascus1 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where   TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())<=29 and TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=15 and id_mesr=".$this->input->post("annio_r").$mes_ini." and microred.id_microred=".$ipress." and celulas_escamosas_atipicas!='_' or celulas_escamosas_atipicas!=  NULL " )->result();
				foreach ($ascus1 as  $values) {	
					$ascus_c1=	$values->cantidad +$ascus_c1;
				}
				$ascus2 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where  TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())<=49 and TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=30 and id_mesr=".$this->input->post("annio_r").$mes_ini." and microred.id_microred=".$ipress." and celulas_escamosas_atipicas!='_' or celulas_escamosas_atipicas!=  NULL " )->result();
				foreach ($ascus2 as  $values) {	
					$ascus_c2=	$values->cantidad +$ascus_c2;
				}
				$ascus3 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where  TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=50 and id_mesr=".$this->input->post("annio_r").$mes_ini."  and microred.id_microred=".$ipress." and celulas_escamosas_atipicas!='_' or celulas_escamosas_atipicas!=  NULL " )->result();
				foreach ($ascus3 as  $values) {	
					$ascus_c3=	$values->cantidad +$ascus_c3;
				}
				$leigb1 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where   TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())<=29 and TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=15 and id_mesr=".$this->input->post("annio_r").$mes_ini."  and microred.id_microred=".$ipress." and leibg!='_' or leibg!=  NULL " )->result();
				foreach ($leigb1 as  $values) {	
					$leibg_c1=	$values->cantidad +$leibg_c1;
				}
				$leigb2 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where  TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())<=49 and TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=30 and id_mesr=".$this->input->post("annio_r").$mes_ini." and microred.id_microred=".$ipress." and leibg!='_' or leibg!=  NULL " )->result();
				foreach ($leigb2 as  $values) {	
					$leibg_c2=	$values->cantidad +$leibg_c2;
				}
				$leigb3 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where  TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=50 and id_mesr=".$this->input->post("annio_r").$mes_ini." and microred.id_microred=".$ipress." and leibg!='_' or leibg!=  NULL " )->result();
				foreach ($leigb3 as  $values) {	
					$leibg_c3=	$values->cantidad +$leibg_c3;
				}
				$leiga1 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where   TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())<=29 and TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=15 and id_mesr=".$this->input->post("annio_r").$mes_ini." and microred.id_microred=".$ipress." and leiag!='_' or leiag!=  NULL " )->result();
				foreach ($leiga1 as  $values) {	
					$leiag_c1=	$values->cantidad +$leiag_c1;
				}
				$leiga2 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where  TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())<=49 and TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=30  and id_mesr=".$this->input->post("annio_r").$mes_ini." and microred.id_microred=".$ipress." and leiag!='_' or leiag!= NULL " )->result();
				foreach ($leiga2 as  $values) {	
					$leiag_c2=	$values->cantidad +$leiag_c2;
				}
				$leiga3 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where  TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=50 and id_mesr=".$this->input->post("annio_r").$mes_ini." and microred.id_microred=".$ipress." and leiag!='_' or leiag!= NULL " )->result();
				foreach ($leiga3 as  $values) {	
					$leiag_c3=	$values->cantidad +$leiag_c3;
				}
				$rechazo1 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where   TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())<=29 and TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=15 and id_mesr=".$this->input->post("annio_r").$mes_ini." and microred.id_microred=".$ipress." and fecha_rechazo!=  '' " )->result();
				foreach ($rechazo1 as  $values) {	
					$rechazo_c1=	$values->cantidad +$rechazo_c1;
				}
				$rechazo2 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where  TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())<=49 and TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=30 and id_mesr=".$this->input->post("annio_r").$mes_ini." and microred.id_microred=".$ipress." and fecha_rechazo!= '' " )->result();
				foreach ($rechazo2 as  $values) {	
					$rechazo_c2=	$values->cantidad +$rechazo_c2;
				}
				$rechazo3 = $this->db->query("select COUNT( TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) ) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where  TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE())>=50 and id_mesr=".$this->input->post("annio_r").$mes_ini."  and microred.id_microred=".$ipress." and fecha_rechazo!='' " )->result();
				foreach ($rechazo3 as  $values) {	
					$rechazo_c3=	$values->cantidad +$rechazo_c3;
				}
				$esta_total = $this->db->query("select count(datos.codigo_renipres) as cantidad FROM  datos INNER JOIN ipress on ipress.codigo=datos.codigo_renipres INNER JOIN microred on microred.id_microred=ipress.microred where id_mesr=".$this->input->post("annio_r").$mes_ini."  and microred.id_microred=".$ipress)->result();	
					foreach ($esta_total as  $value_total) {
						$objPHPExcel->getActiveSheet()->setCellValue('B'.($sitio+$pos), $value_total->cantidad);	
						$objPHPExcel->getActiveSheet()->setCellValue('C'.($sitio+$pos),'='.($value_total->cantidad).'*10/100');
						$objPHPExcel->getActiveSheet()->setCellValue('D'.($sitio+$pos),'='.($value_total->cantidad).'*15/100');
